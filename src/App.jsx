...
try {
  // 👇 TIMPA/PASTE KODE FIREBASE ANDA DI SINI 👇
  ...
  if (firebaseConfig.apiKey && firebaseConfig.apiKey !== 'KODE_ANDA_DISINI') {
    app = initializeApp(firebaseConfig);
    auth = getAuth(app);
    db = getFirestore(app);
    appId = 'docufield-pro-app';
      ...
      <div className="h-52 sm:h-60 bg-slate-50 flex items-center justify-center relative border-b border-slate-100 overflow-hidden">
        {p?.src ? (
          <>
            ...
            <button
              onClick={clearPhoto}
              className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-red-500 text-white p-2 sm:p-2.5 rounded-xl sm:rounded-2xl shadow-xl hover:bg-red-600 transition-all active:scale-90 z-10"
            >
          ...
          <div className="flex flex-col gap-3 w-full px-6 sm:px-10 text-center">
            <label
              className={`w-full text-white py-3 sm:py-4 rounded-2xl sm:rounded-3xl text-[10px] font-black uppercase cursor-pointer flex items-center justify-center gap-2 shadow-lg active:scale-95 transition-all ${
                reportType === 'progres'
          ...
          <>
            <div className="flex bg-slate-100 p-1 rounded-xl sm:rounded-2xl">
              <button
                ...
                className={`flex-1 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black uppercase rounded-lg sm:rounded-xl transition-all ${
                  tab === 'filter'
                ...
                <>
                    ...
                    <div className="bg-emerald-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-emerald-100 shadow-inner">
                      <div className="flex items-center justify-between mb-2 text-[9px] sm:text-[10px] font-black text-emerald-600 uppercase tracking-widest">
                        ...
                        <input
                          ...
                          className="w-12 sm:w-16 p-0.5 sm:p-1 text-center bg-white border border-emerald-200 rounded-md font-black text-xs outline-none"
                        />
                      </div>
                      <input
                        type="range"
                        ...
                        value={progress}
                        onChange={(e) =>
                          updatePhoto('progress', parseInt(e.target.value))
                        }
                        className="w-full h-1.5 sm:h-2 bg-emerald-200 rounded-lg appearance-none cursor-pointer accent-emerald-600"
                      />
                    </div>
                  )}
                  <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-inner">
                    <div className="flex items-center justify-between mb-2 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">
                      <div className="flex items-center gap-1.5">
                        <Sun size={12} className="text-orange-400" /> KECERAHAN
                      </div>
                    ...
                    <input
                      ...
                      value={brightness}
                      onChange={(e) =>
                        updatePhoto('brightness', parseInt(e.target.value))
                      }
                      className="w-full h-1.5 sm:h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                    />
                  </div>
                  <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-inner">
                    <div className="flex items-center justify-between mb-2 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    ...
                    <input
                      ...
                      value={saturation}
                      onChange={(e) =>
                        updatePhoto('saturation', parseInt(e.target.value))
                      }
                      className="w-full h-1.5 sm:h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-emerald-500"
                    />
                  </div>
                </>
              )}
                ...
                <>
                  <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-inner">
                    ...
                    <input
                      ...
                      value={zoom}
                      onChange={(e) =>
                        updatePhoto('zoom', parseInt(e.target.value))
                      }
                      className="w-full h-1.5 sm:h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-blue-600"
                    />
                  </div>
                  <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-inner">
                    ...
                    <input
                      ...
                      value={panX}
                      onChange={(e) =>
                        updatePhoto('panX', parseInt(e.target.value))
                      }
                      className="w-full h-1.5 sm:h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-slate-600"
                    />
                  </div>
                  <div className="bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-100 shadow-inner">
                    <div className="flex items-center justify-between mb-2 text-[9px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest">
                    ...
                    <input
                      ...
                      value={panY}
                      onChange={(e) =>
                        updatePhoto('panY', parseInt(e.target.value))
                      }
                      className="w-full h-1.5 sm:h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer accent-slate-600"
                    />
                  </div>
...
const App = () => {
  ...
  const setPages = (updater) => {
    setPagesData((prev) => ({
      ...prev,
      [reportType]:
        typeof updater === 'function' ? updater(prev[reportType]) : updater,
    }));
  };
  ...
  const [isLibraryReady, setIsLibraryReady] = useState({
    pdf: false,
    ppt: false,
  });
  ...
  const currentAllowed = useMemo(
    () =>
      Array.from(
        new Set([
          ...DEFAULT_EMAIL_IZIN.map((e) => e.toLowerCase()),
          ...(accessData?.allowed || []).map((e) => e.toLowerCase()),
        ])
      ),
    [accessData?.allowed]
  );
  const currentAdmins = useMemo(
    () =>
      Array.from(
        new Set([
          ...DEFAULT_ADMIN.map((e) => e.toLowerCase()),
          ...(accessData?.admins || []).map((e) => e.toLowerCase()),
        ])
      ),
    [accessData?.admins]
  );
    ...
    const fetchSession = async () => {
      try {
        const snap = await getDoc(
          doc(db, 'artifacts', appId, 'users', user.uid, 'session', 'active')
        );
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (
        snap.exists() &&
        snap.data().allowed &&
        Array.isArray(snap.data().allowed)
      ) {
  ...
  const handleAddNewEmail = () => {
    ...
    const newData = {
      allowed: [...currentAllowed, email],
      admins: [...currentAdmins],
    };
    if (
      newEmailAdmin &&
      !newData.admins.map((e) => e.toLowerCase()).includes(email)
    ) {
  ...
  const handleRemoveAccessEmail = (emailToRemove) => {
    ...
    if (DEFAULT_EMAIL_IZIN.includes(emailToRemove.toLowerCase())) {
      setStatusMsg({
        text: 'Email bawaan KODE tidak bisa dihapus.',
        type: 'error',
      });
    ...
    const newData = {
      allowed: currentAllowed.filter(
        (e) => e.toLowerCase() !== emailToRemove.toLowerCase()
      ),
      admins: currentAdmins.filter(
        (e) => e.toLowerCase() !== emailToRemove.toLowerCase()
      ),
    };
    saveAccessToCloud(newData);
  ...
  const saveAccessToCloud = async (newData) => {
    try {
      ...
      setStatusMsg({ text: 'Gagal memperbarui', type: 'error' });
    }
  };
  ...
  const createNewProject = async () => {
    ...
    if (user)
      saveToCloudNow(newId, newInfo, newPages, 'umum', activeEmail, now);
  };

  const openProject = async (project, sessionData = null) => {
    if (!user) {
      setStatusMsg({ text: 'Mode Offline', type: 'error' });
      return;
    }
    ...
    try {
      ...
      const pAuthor =
        freshProjectData.authorEmail || project.authorEmail || activeEmail;
      const pTime =
        freshProjectData.updatedAt || project.updatedAt || Date.now();
      ...
      setReportInfo(
        sessionData?.reportInfo ||
          freshProjectData.reportInfo ||
          defaultReportInfo
      );
      const freshType =
        sessionData?.reportType ||
        freshProjectData.lastActiveTab ||
        freshProjectData.reportType ||
        'umum';
      ...
      let loadedProgres = [];
      const isLegacy =
        freshProjectData.pageCount !== undefined &&
        freshProjectData.pageCountUmum === undefined;
      ...
      if (isLegacy) {
        ...
        snaps.forEach((pSnap) => {
          if (pSnap.exists()) {
            if (freshProjectData.reportType === 'progres')
              loadedProgres.push(pSnap.data().data);
        ...
        umumSnaps.forEach((pSnap) => {
          if (pSnap.exists()) loadedUmum.push(pSnap.data().data);
        });
        progresSnaps.forEach((pSnap) => {
          if (pSnap.exists()) loadedProgres.push(pSnap.data().data);
        });
      }
  ...
  useEffect(() => {
    ...
    if (!isLibraryReady.pdf) {
      Promise.all([
        ...
        setIsLibraryReady({ pdf: !!window.html2pdf, ppt: !!window.PptxGenJS })
      );
    }
  }, [isLibraryReady.pdf]);
  ...
  const downloadMentahan = () => {
    try {
      ...
      a.href = url;
      a.download = `Mentahan_${reportInfo.project || 'Data'}.json`;
      a.click();
      setStatusMsg({ text: 'Mentahan Disimpan!', type: 'success' });
    } catch {
      setStatusMsg({ text: 'Gagal simpan', type: 'error' });
    }
    setTimeout(() => setStatusMsg({ text: '', type: '' }), 2000);
  };
  ...
  const loadMentahan = (e) => {
    ...
    reader.onload = (ev) => {
      try {
        ...
        if (!loadedPages) {
          loadedPages =
            data.reportType === 'progres'
              ? {
                  umum: [createNewPage()],
                  progres: data.pages || [createNewPage()],
                }
              : {
                  umum: data.pages || [createNewPage()],
                  progres: [createNewPage()],
                };
        }

        setReportInfo(loadedInfo);
        setReportType(loadedType);
    ...
    if (pendingAction === 'dashboard') {
      ...
      if (activeProjectId) {
        ...
        setProjects((prevProjects) => {
          const existingIdx = prevProjects.findIndex(
  ...
  const bakeImageFilters = (
    ...
    return new Promise((resolve) => {
      ...
      img.onload = () => {
        ...
        const nw = sW * scale;
        const nh = sH * scale;
        const finalX = sX + (sW - nw) * (panX / 100);
        const finalY = sY + (sH - nh) * (panY / 100);
  ...
  const updateSpecificPhoto = (pIdx, sIdx, key, val) => {
    setPages((prev) => {
      const n = [...prev];
      if (!n[pIdx]) return prev;
      const npo = [...n[pIdx]];
      npo[sIdx] = { ...npo[sIdx], [key]: val };
      n[pIdx] = npo;
      return n;
    });
  };
  ...
  const handleMegaUpload = async (e) => {
    ...
    let newPageRef = [...updatedPages[curIdx]];
    const emptySlots = [];
    newPageRef.forEach((s, i) => {
      if (!s.src) emptySlots.push(i);
    });
    ...
    for (let i = 0; i < limit; i++) {
      ...
      const p = new Promise((r) => {
        ...
        reader.onload = async (ev) =>
          r(await processInitialUpload(ev.target.result));
      });
      const cropped = await p;
      if (cropped)
        newPageRef[emptySlots[i]] = {
          ...newPageRef[emptySlots[i]],
          src: cropped,
          brightness: 100,
          saturation: 100,
    ...
    updatedPages[curIdx] = newPageRef;
  ...
  const handleLogoUpload = async (idx, e) => {
    ...
    reader.readAsDataURL(file);
    reader.onload = async (ev) => {
      const cropped = await processLogoUpload(ev.target.result);
      if (cropped) {
  ...
  const switchTab = (targetMode) => {
    ...
    setReportInfo({
      ...
      title:
        targetMode === 'progres'
          ? 'LAPORAN DOKUMENTASI PROGRES LAPANGAN'
  ...
  const triggerPdfBaking = async () => {
    ...
    setStatusMsg({ text: 'Menyiapkan PDF...', type: 'info' });
    try {
        ...
        pages.map(async (page) => {
          return await Promise.all(
            page.map(async (photo) => {
              ...
              const finalSrc = await bakeImageFilters(
                photo.src,
                photo.brightness,
                photo.saturation,
                photo.zoom,
  ...
  useEffect(() => {
    if (shouldTriggerDownload && bakedPages) {
      const generatePDF = async () => {
        ...
        const options = {
          margin: 0,
          filename: `${reportInfo.title.replace(/ /g, '_')}.pdf`,
  ...
  const downloadPPTX = async () => {
    ...
    setStatusMsg({ text: 'Membangun PPTX Identik...', type: 'info' });
    try {
      const pptx = new window.PptxGenJS();
      pptx.defineLayout({ name: 'A4', width: 8.27, height: 11.69 });
      pptx.layout = 'A4';
      const PAGE_MARGIN_TOP = 0.236,
        PAGE_MARGIN_SIDE = 0.59,
        CONTENT_W = 7.09;
      ...
      for (let i = 0; i < 6; i++) {
        const col = i % 2,
          row = Math.floor(i / 2);
        const x = PAGE_MARGIN_SIDE + col * (BOX_W + GAP),
          y = gridStartY + row * (BOX_H + 0.15);
        const imgX = x + 0.1,
          imgY = y + 0.1,
          imgW = BOX_W - 0.2,
          imgH = 1.97;
      ...
      for (const pData of pages) {
        ...
        if (reportInfo.logos?.some((l) => l !== null)) {
          const lw = 2.36,
            lh = 0.5;
          if (reportInfo.logos[0])
            slide.addImage({
              data: reportInfo.logos[0],
              x: PAGE_MARGIN_SIDE,
              y: curY,
              w: lw,
              h: lh,
          ...
          if (reportInfo.logos[1])
            slide.addImage({
              data: reportInfo.logos[1],
              x: 8.27 / 2 - lw / 2,
              y: curY,
              w: lw,
              h: lh,
          ...
          if (reportInfo.logos[2])
            slide.addImage({
              data: reportInfo.logos[2],
              x: 8.27 - PAGE_MARGIN_SIDE - lw,
              y: curY,
              w: lw,
              h: lh,
              sizing: { type: 'contain' },
        ...
        curY += 0.65;
        slide.addText(reportInfo.title.toUpperCase(), {
          x: PAGE_MARGIN_SIDE,
          y: curY,
          w: CONTENT_W,
          fontSize: 16,
        ...
        curY += 0.45;
        const meta = [
          { l: 'PEKERJAAN', v: reportInfo.project },
          { l: 'INSTANSI', v: reportInfo.department },
          { l: 'KONTRAKTOR', v: reportInfo.contractor || reportInfo.company },
          { l: 'KONSULTAN', v: reportInfo.consultant },
        ...
        if (reportType === 'progres')
          lineColor = isModernTemplate ? '059669' : '10B981'; // Emerald
        ...
        slide.addShape(pptx.ShapeType.line, {
          ...
          w: CONTENT_W,
          h: 0,
          line: { color: lineColor, width: isModernTemplate ? 3 : 2 },
        });
        ...
        for (let i = 0; i < pData.length; i++) {
          ...
          const x = PAGE_MARGIN_SIDE + col * (BOX_W + GAP),
            y = gridStartY + row * (BOX_H + 0.15);
          const imgW = BOX_W - 0.2,
            imgH = 1.97;

          if (photo && photo.src) {
            const finalImg = await bakeImageFilters(
              photo.src,
              photo.brightness,
              photo.saturation,
              photo.zoom,
          ...
          if (reportType === 'progres' && photo?.src) {
            ...
            slide.addText(`PROGRES: ${photo.progress || 0}%`, {
              x: x + BOX_W - 0.9,
              y: noteY,
              w: 0.8,
              h: 0.15,
      ...
      await pptx.writeFile({
        fileName: `${reportInfo.title.replace(/ /g, '_')}.pptx`,
      });
      setIsPptLoading(false);
      setStatusMsg({ text: 'PPTX Identik Siap!', type: 'success' });
    } catch {
      setIsPptLoading(false);
      setStatusMsg({ text: 'Gagal PPTX', type: 'error' });
    }
  };
  ...
  const ReportPage = ({ data, isFinal = false }) => {
    ...
    const baseFontClass = isModern
      ? 'font-serif text-slate-800'
      : 'font-sans text-black';
    const headerTitleClass = isModern
      ? 'text-indigo-950 tracking-wide font-bold'
    ...
    const cardContainerClass = isModern
      ? 'border border-indigo-100 shadow-md rounded-2xl'
    ...
    if (isModern) {
      noteBorderClass =
        reportType === 'progres' ? 'border-emerald-600' : 'border-indigo-500';
    } else {
      noteBorderClass =
        reportType === 'progres' ? 'border-emerald-500' : 'border-blue-500';
    }

    return (
      <div
        className={`bg-white w-[210mm] flex flex-col ${baseFontClass} relative box-border ${
          isFinal
            ? 'report-page-final'
            : 'mb-10 shadow-2xl rounded-2xl border border-slate-200 shrink-0'
        }`}
        style={{
          height: '296.7mm',
      ...
      >
        <div className={`text-center pb-4 mb-5 flex-none ${headerBorderClass}`}>
          ...
          <h2
            className={`text-xl uppercase mb-4 leading-tight ${headerTitleClass}`}
          >
            {reportInfo.title}
          </h2>
          <div className="text-left space-y-0.5">
            {meta.map((m, idx) => (
              <div
                key={idx}
                className="flex items-start uppercase text-[7pt] tracking-tight"
              >
                <span className="w-24 shrink-0 font-bold text-slate-400">
                  {m.l}
                </span>
                <span className="mr-2 font-bold text-slate-600">:</span>
                <span className="font-black flex-1 break-words leading-tight text-slate-800">
                  {m.v || '-'}
                </span>
              </div>
            ))}
        ...
        <div className="grid grid-cols-2 gap-4 flex-grow content-start">
          {data.map((p, i) => (
            <div
              key={i}
              className={`p-2.5 flex flex-col h-[72mm] bg-white box-border ${cardContainerClass}`}
            >
              <div
                className={`h-[50mm] bg-slate-50 relative overflow-hidden flex items-center justify-center border border-slate-100 ${imgContainerClass}`}
              >
                {p?.src ? (
                  <img
                    src={p.src}
                    ...
                    style={{
                      filter: `brightness(${p.brightness || 100}%) saturate(${
                        p.saturation || 100
                      }%)`,
                      transform: `scale(${(p.zoom || 100) / 100})`,
                      transformOrigin: `${p.panX ?? 50}% ${p.panY ?? 50}%`,
                    }}
                    alt=""
                  />
                ) : (
              ...
              <div
                className={`mt-2.5 border-l-4 pl-3 overflow-hidden flex-1 ${noteBorderClass}`}
              >
                <div className="flex items-center justify-between mb-0.5">
                  <div className="text-[6pt] font-black text-slate-300 uppercase tracking-tighter">
                    KETERANGAN:
                  </div>
                  {reportType === 'progres' && p?.src && (
                    <div className="text-[7pt] font-black text-slate-900 bg-slate-50 px-1.5 py-0.5 rounded border border-slate-200">
                      PROGRES: {p.progress || 0}%
                    </div>
                  )}
                </div>
                <p
                  className={`text-[8.5pt] leading-tight italic line-clamp-2 ${
                    isModern ? 'text-slate-800' : 'text-slate-700 font-medium'
                  }`}
                >
                  {p?.note || '-'}
                </p>
  ...
  if (!isFirebaseConfigured) {
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-6 relative overflow-hidden">
        <div className="absolute top-[-20%] left-[-10%] w-[60%] h-[60%] bg-red-600 rounded-full mix-blend-screen filter blur-[150px] opacity-20 animate-pulse"></div>
        <div className="bg-red-500/10 p-6 rounded-full mb-6 border border-red-500/20 shadow-[0_0_50px_rgba(239,68,68,0.3)] z-10">
          <AlertCircle size={48} className="text-red-500" />
        </div>
        <h1 className="text-2xl sm:text-3xl font-black text-white mb-4 uppercase tracking-widest text-center z-10">
          Tindakan Diperlukan
        </h1>
        <div className="bg-slate-800 border border-slate-700 p-6 sm:p-8 rounded-[32px] max-w-lg text-center shadow-2xl z-10">
          <p className="text-slate-300 leading-relaxed mb-6 text-sm sm:text-base font-medium">
            Aplikasi mendeteksi bahwa konfigurasi Firebase masih menggunakan
            tulisan bawaan (
            <code className="text-pink-400 bg-pink-400/10 px-2 py-1 rounded font-mono text-xs">
              KODE_ANDA_DISINI
            </code>
            ).
          </p>
          <div className="bg-slate-900/50 border border-slate-700/50 p-4 rounded-2xl text-left">
            <p className="text-slate-400 text-xs sm:text-sm font-medium flex gap-2 items-start">
              <span className="text-blue-400 shrink-0">💡</span>
              <span>
                Silakan salin{' '}
                <strong className="text-white">firebaseConfig</strong> dari
                Firebase Console Anda, lalu paste dan timpa tulisan tersebut di
                dalam file <strong className="text-white">App.jsx</strong>{' '}
                (sekitar baris 15-22).
              </span>
            </p>
          </div>
    ...
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center p-4 relative overflow-hidden">
        ...
        <div className="bg-white p-8 sm:p-10 rounded-[32px] sm:rounded-[40px] shadow-2xl w-full max-w-md z-10 border border-slate-200">
          ...
          <h1 className="text-2xl font-black text-center text-slate-800 mb-2 uppercase">
            Akses Terbatas
          </h1>
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            <div>
              ...
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl sm:rounded-3xl font-medium outline-none transition-all shadow-inner text-sm"
                  placeholder="admin@proyek.com"
                />
              </div>
            </div>
            {loginError && (
              <div className="bg-red-50 text-red-600 p-4 rounded-2xl text-xs font-bold border border-red-100 flex items-start gap-2">
            ...
            <button
              type="submit"
              disabled={!isAppReady}
              className="w-full bg-blue-600 hover:bg-blue-700 text-white py-4 rounded-2xl sm:rounded-3xl font-black uppercase tracking-widest shadow-xl shadow-blue-500/30 active:scale-95 transition-all text-sm disabled:opacity-50"
            >
  ...
  if (!isAppReady) {
    return (
      <div className="min-h-screen bg-slate-100 flex flex-col items-center justify-center">
        <Cloud size={56} className="text-blue-500 mb-6 animate-pulse" />
  ...
  return (
    <div className="min-h-screen bg-slate-100 font-sans pb-20 overflow-x-hidden relative">
        ...
        <div className="flex items-center justify-between w-full sm:w-auto">
          <div className="flex items-center gap-2.5">
            <Cloud
              size={22}
              className={
                reportType === 'progres' ? 'text-emerald-400' : 'text-blue-400'
              }
            />
              ...
              <>
                ...
                {saveStatus === 'error' && (
                  <button
                    ...
                    className="text-red-400 hover:text-red-300 transition-all flex items-center gap-1 bg-red-400/10 px-2 py-1 rounded-md"
                  >
        ...
        <div className="flex gap-2 items-center w-full sm:w-auto overflow-x-auto pb-1 sm:pb-0 scrollbar-hide">
          {view !== 'dashboard' && (
            <>
              <button
                ...
                className="shrink-0 flex items-center gap-1.5 sm:gap-2 bg-white/10 hover:bg-white/20 px-3 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-bold transition-all"
              >
                <ArrowLeft size={14} />{' '}
              ...
              <button
                ...
                className={`shrink-0 px-3 py-2 sm:px-5 sm:py-2.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black transition-all ${
                  view === 'edit'
              ...
              <button
                onClick={downloadMentahan}
                className="shrink-0 bg-slate-700 hover:bg-slate-600 px-3 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs text-white flex items-center gap-1.5 sm:gap-2 shadow-lg transition-all"
                title="Simpan Mentahan (.json)"
              >
                <HardDrive size={16} className="w-4 h-4 sm:w-5 sm:h-5" />{' '}
                <span className="hidden lg:inline">MENTAHAN (.json)</span>
              </button>
              <button
                onClick={triggerPdfBaking}
                disabled={isPdfLoading}
                className="shrink-0 bg-emerald-600 hover:bg-emerald-500 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs text-white flex items-center gap-1.5 sm:gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-50"
              >
                {isPdfLoading ? (
                  <Loader2
                    size={16}
                  ...
                  <FileDown size={16} className="w-4 h-4 sm:w-5 sm:h-5" />
                )}{' '}
                PDF
              </button>
              <button
                ...
                className="shrink-0 bg-orange-600 hover:bg-orange-500 px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs text-white flex items-center gap-1.5 sm:gap-2 shadow-lg transition-all active:scale-95 disabled:opacity-50"
              >
                {isPptLoading ? (
                  <Loader2
                    size={16}
                  ...
                  <Presentation size={16} className="w-4 h-4 sm:w-5 sm:h-5" />
                )}{' '}
                PPTX
              </button>
            </>
          ...
          {view === 'dashboard' && (
            <button
              ...
              className="shrink-0 bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white px-3 py-2 sm:px-4 sm:py-2.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black uppercase flex items-center gap-1.5 sm:gap-2 transition-all ml-auto"
            >
      ...
      {statusMsg.text && (
        <div
          className={`fixed top-20 sm:top-24 left-1/2 -translate-x-1/2 z-[100] px-6 py-2.5 sm:px-8 sm:py-3.5 rounded-2xl sm:rounded-3xl shadow-2xl flex items-center gap-3 sm:gap-4 font-black text-[10px] sm:text-xs text-white uppercase tracking-widest animate-in slide-in-from-top-6 ${
      ...
      {view === 'dashboard' && (
        <main className="max-w-6xl mx-auto p-4 sm:p-8 animate-in fade-in duration-500">
          <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end mb-8 sm:mb-10 gap-6">
            <div>
              <h2 className="text-2xl sm:text-3xl font-black text-slate-900 mb-2 flex items-center gap-2.5 sm:gap-3">
                <FolderOpen className="text-blue-600 w-8 h-8 sm:w-9 sm:h-9" />{' '}
                Arsip Laporan
              </h2>
              <p className="text-slate-500 font-medium flex items-center gap-2 flex-wrap text-xs sm:text-sm">
                Masuk sebagai:{' '}
                <span className="bg-blue-100 text-blue-700 px-2 py-0.5 rounded-md font-bold">
                  {activeEmail}
                </span>
                {isAdmin && (
                  <span className="bg-red-100 text-red-700 px-2 py-0.5 rounded-md font-bold text-[9px] sm:text-[10px] uppercase shadow-sm">
                    👑 Admin
                  </span>
                )}
              </p>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full lg:w-auto">
              {isAdmin && (
                <button
                  ...
                  className="w-full sm:w-auto bg-slate-800 text-white hover:bg-slate-700 px-4 sm:px-6 py-3.5 sm:py-4 rounded-2xl sm:rounded-[24px] font-black uppercase text-[10px] sm:text-xs flex items-center justify-center gap-2 shadow-lg transition-all active:scale-95"
                >
                  <User size={16} className="sm:w-[18px] sm:h-[18px]" /> AKSES
                </button>
              )}
              <label className="w-full sm:w-auto bg-white text-blue-600 border-2 border-blue-600 hover:bg-blue-50 px-4 sm:px-6 py-3.5 sm:py-4 rounded-2xl sm:rounded-[24px] font-black uppercase text-[10px] sm:text-xs flex items-center justify-center gap-2 sm:gap-3 shadow-lg cursor-pointer transition-all active:scale-95">
                <UploadCloud size={16} className="sm:w-5 sm:h-5" /> BUKA
                MENTAHAN
                <input
                  type="file"
                  accept=".json"
                  onChange={loadMentahan}
                  className="hidden"
                />
              </label>
              <button
                onClick={createNewProject}
                className="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white px-4 sm:px-8 py-3.5 sm:py-4 rounded-2xl sm:rounded-[24px] font-black uppercase text-[10px] sm:text-xs flex items-center justify-center gap-2 sm:gap-3 shadow-xl shadow-blue-500/30 transition-all active:scale-95"
              >
                <Plus size={16} className="sm:w-5 sm:h-5" /> BUAT LAPORAN
              </button>
            </div>
          </div>
          {!user ? (
            <div className="bg-orange-50 rounded-[32px] sm:rounded-[40px] border-2 border-dashed border-orange-200 p-8 sm:p-10 flex flex-col items-center justify-center text-center">
              <WifiOff
                size={40}
                className="text-orange-400 mb-4 sm:mb-6 sm:w-12 sm:h-12"
              />
              <h3 className="text-lg sm:text-xl font-black text-orange-700 mb-2">
                Offline
              </h3>
              <p className="text-orange-600 max-w-lg mb-6 text-xs sm:text-sm">
                Pekerjaan tidak tersinkron. Gunakan BUKA MENTAHAN atau buat
                baru, dan ingat klik MENTAHAN untuk backup lokal.
              </p>
            </div>
          ) : projects.length === 0 ? (
            <div className="bg-white rounded-[32px] sm:rounded-[40px] border-2 border-dashed border-slate-300 p-10 sm:p-16 flex flex-col items-center justify-center text-center">
              <ClipboardList
                size={48}
                className="text-slate-300 mb-4 sm:mb-6 sm:w-16 sm:h-16"
              />
              <h3 className="text-lg sm:text-xl font-black text-slate-600 mb-2">
                Belum Ada Laporan
              </h3>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
              {projects.map((p) => (
                <div
                  key={p.id}
                  className="bg-white rounded-[24px] sm:rounded-[32px] p-5 sm:p-6 shadow-xl border border-slate-200 hover:border-blue-400 transition-all flex flex-col hover:-translate-y-1"
                >
                  <div className="flex justify-between items-start mb-3 sm:mb-4">
                    <span className="px-2 sm:px-3 py-1 rounded-lg sm:rounded-xl text-[9px] sm:text-[10px] font-black uppercase bg-blue-100 text-blue-700">
                      DOKUMENTASI
                    </span>
                    <button
                      onClick={(e) => {
                        e.stopPropagation();
                        setShowDeleteProjectModal({ show: true, project: p });
                      }}
                      className="p-1.5 sm:p-2 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded-lg sm:rounded-xl transition-all"
                    >
                      <Trash2 size={16} className="sm:w-[18px] sm:h-[18px]" />
                    </button>
                  </div>
                  <h3 className="text-base sm:text-lg font-black text-slate-800 mb-3 sm:mb-4 line-clamp-2">
                    {p.reportInfo?.title || 'Laporan'}
                  </h3>
                  <div className="space-y-1.5 sm:space-y-2 mb-4 sm:mb-6 flex-grow text-[10px] sm:text-xs text-slate-500 font-medium">
                    <div className="flex items-center gap-2 truncate">
                      <Briefcase
                        size={12}
                        className="sm:w-3.5 sm:h-3.5 text-slate-400"
                      />{' '}
                      {p.reportInfo?.project || '-'}
                    </div>
                    ...
                    <div className="flex items-center gap-2">
                      <FileText
                        ...
                        className="sm:w-3.5 sm:h-3.5 text-slate-400"
                      />{' '}
                      {(p.pageCountUmum || 0) + (p.pageCountProgres || 0) ||
                        p.pageCount ||
                        0}{' '}
                      Halaman
                    </div>
                  ...
                  <button
                    ...
                    className="w-full bg-slate-50 hover:bg-slate-100 text-slate-700 border border-slate-200 py-2.5 sm:py-3 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs uppercase transition-all flex justify-center items-center gap-2"
                  >
                    Buka Laporan{' '}
        ...
        <main className="max-w-6xl mx-auto p-4 sm:p-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
          <div className="flex bg-white p-1.5 sm:p-2 rounded-[24px] sm:rounded-[32px] mb-6 sm:mb-8 shadow-xl max-w-md mx-auto border border-slate-200">
            <button
              ...
              className={`flex-1 flex justify-center items-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 rounded-2xl sm:rounded-3xl text-[9px] sm:text-[10px] font-black uppercase transition-all ${
                reportType === 'umum'
                  ? 'bg-blue-600 text-white shadow-md'
                  : 'text-slate-400 hover:bg-slate-50'
              }`}
            >
              <ClipboardList size={14} /> Umum
            </button>
            <button
              onClick={() => switchTab('progres')}
              className={`flex-1 flex justify-center items-center gap-1.5 sm:gap-2 py-2.5 sm:py-3 rounded-2xl sm:rounded-3xl text-[9px] sm:text-[10px] font-black uppercase transition-all ${
                reportType === 'progres'
          ...
          <section className="bg-white rounded-[32px] sm:rounded-[40px] p-5 sm:p-8 shadow-2xl mb-8 sm:mb-10 border border-slate-200/60">
            <div className="mb-6 sm:mb-8">
              <label className="text-[9px] sm:text-[10px] font-black text-slate-400 tracking-widest ml-2 block mb-2 sm:mb-3 uppercase">
                Logo Header (3 Slot)
              </label>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
                {[0, 1, 2].map((idx) => (
                  <div
                    key={idx}
                    className="relative h-16 sm:h-20 bg-slate-50 border-2 border-dashed border-slate-200 rounded-xl sm:rounded-2xl flex items-center justify-center overflow-hidden group"
                  >
                    {reportInfo.logos?.[idx] ? (
                      <>
                        <img
                          src={reportInfo.logos[idx]}
                          className="h-full w-full object-contain p-1 sm:p-2"
                          alt=""
                        />
                        <button
                          onClick={() => removeLogo(idx)}
                          className="absolute top-1.5 right-1.5 sm:top-2 sm:right-2 bg-red-500 text-white rounded-md sm:rounded-lg p-1 sm:p-1.5 shadow-md opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
                        >
                          <Trash2 size={12} />
                        </button>
                      </>
                      ...
                      <label className="cursor-pointer flex flex-col items-center justify-center w-full h-full text-slate-400 hover:bg-slate-100 transition-all">
                        <Upload
                          size={14}
                          className="mb-0.5 sm:mb-1 sm:w-4 sm:h-4"
                        />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6 uppercase">
              <div className="md:col-span-2">
                <label className="text-[9px] sm:text-[10px] font-black text-slate-400 tracking-widest ml-2">
                  Judul Laporan
                </label>
                <input
                  type="text"
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReportInfo({ ...reportInfo, project: e.target.value })
                  }
                  className="w-full p-3.5 sm:p-4 bg-slate-50 border-2 border-transparent rounded-2xl sm:rounded-3xl font-black text-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner text-sm"
                />
              </div>
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setReportInfo({ ...reportInfo, department: e.target.value })
                  }
                  className="w-full p-3.5 sm:p-4 bg-slate-50 border-2 border-transparent rounded-2xl sm:rounded-3xl font-black text-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner text-sm"
                />
              </div>
              ...
              <div>
                ...
                <input
                  type="text"
                  value={reportInfo.contractor || ''}
                  onChange={(e) =>
                    setReportInfo({ ...reportInfo, contractor: e.target.value })
                  }
                  className="w-full p-3.5 sm:p-4 bg-slate-50 border-2 border-transparent rounded-2xl sm:rounded-3xl font-black text-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner text-sm"
                />
              </div>
              <div>
                ...
                <input
                  type="text"
                  value={reportInfo.consultant || ''}
                  onChange={(e) =>
                    setReportInfo({ ...reportInfo, consultant: e.target.value })
                  }
                  className="w-full p-3.5 sm:p-4 bg-slate-50 border-2 border-transparent rounded-2xl sm:rounded-3xl font-black text-slate-800 focus:border-blue-500 outline-none transition-all shadow-inner text-sm"
                />
              </div>
              ...
              <div className="md:col-span-2 mt-4 pt-4 border-t border-slate-100">
                <label className="text-[9px] sm:text-[10px] font-black text-blue-500 tracking-widest ml-2 flex items-center gap-1.5 mb-2">
                  <Palette size={14} /> TEMA TAMPILAN PDF
                </label>
                <div className="flex gap-3 bg-slate-50 p-2 rounded-2xl sm:rounded-3xl shadow-inner border border-slate-200">
                  <button
                    onClick={() =>
                      setReportInfo({ ...reportInfo, template: 'klasik' })
                    }
                    className={`flex-1 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black transition-all ${
                      !reportInfo.template || reportInfo.template === 'klasik'
                  ...
                  <button
                    onClick={() =>
                      setReportInfo({ ...reportInfo, template: 'modern' })
                    }
                    className={`flex-1 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[10px] sm:text-xs font-black transition-all ${
                      reportInfo.template === 'modern'
                        ? 'bg-indigo-600 shadow-md text-white border border-indigo-700'
                        : 'text-slate-400 hover:text-slate-600 hover:bg-slate-100'
                    }`}
                  >
                    Desain Modern
            ...
            <div className="flex flex-col lg:flex-row items-center gap-4 sm:gap-6 bg-slate-900 rounded-[24px] sm:rounded-[32px] p-4 sm:p-5 text-white mt-8 sm:mt-10 shadow-xl border border-white/5 w-full">
              <div className="flex items-center justify-between w-full lg:w-max shrink-0">
                <button
                  ...
                  className="p-2.5 sm:p-3 bg-white/10 rounded-xl sm:rounded-2xl hover:bg-white/20 transition-all active:scale-90"
                >
                  <ChevronLeft size={20} className="sm:w-6 sm:h-6" />
                </button>
                <span className="font-black text-lg sm:text-xl tracking-tighter mx-4 whitespace-nowrap">
                  {currentPage}{' '}
              ...
              <div className="flex gap-2 sm:gap-3 w-full lg:w-auto flex-wrap justify-center lg:justify-end lg:ml-auto">
                {pages.length > 1 && (
                  <button
                    ...
                    className="flex-1 sm:flex-none bg-red-600 hover:bg-red-500 text-white px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase flex items-center justify-center gap-1.5 sm:gap-2 shadow-md transition-all active:scale-95 whitespace-nowrap"
                  >
                    <Trash2 size={14} className="sm:w-4 sm:h-4" /> HAPUS HAL
                  </button>
                )}
                <button
                  onClick={executeDeleteAllPhotos}
                  className="flex-1 sm:flex-none bg-orange-500 hover:bg-orange-400 text-white px-3 sm:px-4 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase flex items-center justify-center gap-1.5 sm:gap-2 shadow-md transition-all active:scale-95 whitespace-nowrap"
                >
                  KOSONGKAN
                </button>
                <label className="flex-1 sm:flex-none bg-white text-slate-900 px-3 sm:px-5 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase cursor-pointer flex items-center justify-center gap-1.5 sm:gap-3 transition-all active:scale-95 shadow-md whitespace-nowrap">
                  <Upload size={14} className="sm:w-4 sm:h-4 text-blue-600" />{' '}
                  MEGA UPLOAD
                  <input
                    type="file"
                    multiple
                    accept="image/*"
                    className="hidden"
                    onChange={handleMegaUpload}
                  />
                </label>
                <button
                  onClick={handleAddPage}
                  className={`w-full sm:w-auto flex-1 sm:flex-none text-white px-4 sm:px-6 py-3 sm:py-3.5 rounded-xl sm:rounded-2xl text-[9px] sm:text-[10px] font-black uppercase transition-all active:scale-95 shadow-lg whitespace-nowrap ${
                    reportType === 'progres' ? 'bg-emerald-600' : 'bg-blue-600'
                  }`}
                >
                  + HALAMAN BARU
                </button>
          ...
          <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
            {activePageData.map((p, i) => (
              <PhotoCard
                ...
                updatePhoto={(k, v) =>
                  updateSpecificPhoto(currentPage - 1, i, k, v)
                }
                clearPhoto={() => clearSpecificPhoto(currentPage - 1, i)}
                handleFileUpload={(e) =>
                  handleFileUpload(currentPage - 1, i, e)
                }
              />
            ))}
          </section>
        ...
        <main className="w-full flex flex-col items-center bg-slate-200/50 min-h-screen relative overflow-x-hidden">
          <div className="sticky top-2 sm:top-4 z-40 bg-slate-900/80 backdrop-blur-xl px-5 py-3 rounded-full flex items-center gap-5 shadow-[0_10px_40px_rgba(0,0,0,0.3)] text-white mt-2 sm:mt-4 mb-4 border border-white/20 transition-all hover:bg-slate-900">
            <button
              ...
              className="p-1 hover:text-blue-400 active:scale-90 transition-all"
            >
              <ZoomOut size={18} />
            </button>
            ...
            <button
              onClick={() =>
                setPreviewZoom(
                  window.innerWidth < 850 ? (window.innerWidth - 20) / 794 : 1
                )
              }
              className="p-1 hover:text-blue-400 active:scale-90 transition-all text-[10px] font-black tracking-widest flex items-center gap-1.5"
            >
          ...
          <div
            ...
            style={{
              ...
              width: '210mm',
              marginBottom: `${(previewZoom - 1) * pages.length * 1122}px`,
            }}
          >
            {pages.map((p, i) => (
              <ReportPage key={`preview-${i}`} data={p} />
            ))}
          </div>
        </main>
      )}
      ...
      >
        ...
        >
          {(bakedPages || pages).map((p, i) => (
            <ReportPage key={`render-${i}`} data={p} isFinal={true} />
          ))}
        </div>
      </div>
          ...
          <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 border-2 border-orange-100">
            <div className="flex items-center gap-3 sm:gap-4 text-orange-500 mb-4">
              <div className="bg-orange-100 p-2 sm:p-3 rounded-full">
                <HardDrive
                  size={24}
                  className="sm:w-7 sm:h-7 text-orange-600"
                />
              </div>
            ...
            <p className="text-slate-600 font-medium mb-6 sm:mb-8 text-xs sm:text-sm leading-relaxed">
              Sangat disarankan untuk mengunduh{' '}
              <strong>File Mentahan (.json)</strong> ke perangkat Anda sebagai
              cadangan sebelum{' '}
              ...
              . File ini dapat dibuka kapan saja meskipun internet terputus.
            </p>
      ...
      {showDeleteProjectModal.show && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[24px] sm:rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center gap-3 sm:gap-4 text-red-600 mb-4">
              <ShieldAlert size={28} className="sm:w-8 sm:h-8" />
              <h3 className="text-lg sm:text-xl font-black uppercase tracking-widest">
                Hapus Proyek
              </h3>
            </div>
            <p className="text-slate-600 font-medium mb-6 sm:mb-8 text-xs sm:text-sm">
              Hapus laporan{' '}
              <strong>
                {showDeleteProjectModal.project?.reportInfo?.title}
              </strong>{' '}
              dari Cloud?
            </p>
            <div className="flex gap-3 sm:gap-4">
              <button
                ...
                className="flex-1 py-3 sm:py-3.5 font-black bg-slate-100 rounded-xl sm:rounded-2xl uppercase text-[10px] sm:text-xs active:scale-95 transition-all text-slate-500"
              >
                Batal
              </button>
              <button
                onClick={executeDeleteProject}
                className="flex-1 py-3 sm:py-3.5 font-black bg-red-600 text-white rounded-xl sm:rounded-2xl uppercase text-[10px] sm:text-xs shadow-lg active:scale-95 transition-all"
              >
                Hapus Total
              </button>
            </div>
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[24px] sm:rounded-3xl p-6 sm:p-8 max-w-sm w-full shadow-2xl animate-in zoom-in-95">
            ...
            <p className="text-slate-600 font-medium mb-6 sm:mb-8 text-xs sm:text-sm">
              ...
              <strong>Halaman {currentPage}</strong> beserta seluruh isinya?
            </p>
            <div className="flex gap-3 sm:gap-4">
              <button
        ...
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
          <div className="bg-white rounded-[24px] sm:rounded-[32px] p-6 sm:p-8 max-w-lg w-full shadow-2xl animate-in zoom-in-95 flex flex-col max-h-[90vh]">
            ...
            <div className="mb-4 sm:mb-6 bg-slate-50 p-3 sm:p-4 rounded-2xl sm:rounded-3xl border border-slate-200">
              <label className="block text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1.5 sm:mb-2 ml-1 sm:ml-2">
                Tambah Email Baru
              </label>
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2.5 sm:px-4 sm:py-3 bg-white border border-slate-200 rounded-xl sm:rounded-2xl text-xs sm:text-sm outline-none"
                />
              </div>
              <div className="flex items-center justify-between mt-2.5 sm:mt-3 px-1 sm:px-2">
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={newEmailAdmin}
                    onChange={(e) => setNewEmailAdmin(e.target.checked)}
                ...
                <button
                  ...
                  className="bg-blue-600 text-white px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-[9px] sm:text-xs font-black uppercase transition-all shadow-md"
                >
                  Tambah
                </button>
              </div>
            ...
            <div className="flex-1 overflow-y-auto pr-1 sm:pr-2 space-y-1.5 sm:space-y-2 mb-3 sm:mb-4">
              <label className="block text-[9px] sm:text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 sm:mb-3 ml-1 sm:ml-2">
                Daftar Terdaftar
              </label>
              {currentAllowed.map((em, idx) => {
                const isDefault = DEFAULT_EMAIL_IZIN.includes(em.toLowerCase());
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between bg-white border border-slate-200 p-2.5 sm:p-3 rounded-xl sm:rounded-2xl shadow-sm"
                  >
                    <div className="flex flex-col">
                      <span className="font-bold text-xs sm:text-sm text-slate-700 truncate max-w-[180px] sm:max-w-none">
                        {em}{' '}
                        {isDefault && (
                          <span className="text-[8px] ml-1 px-1.5 py-0.5 bg-slate-200 text-slate-500 rounded">
                            KODE DEFAULT
                          </span>
                        )}
                      </span>
                      {currentAdmins.includes(em) && (
                        <span className="text-[8px] sm:text-[9px] font-black text-red-500 uppercase tracking-widest mt-0.5">
                          Administrator
                        </span>
                      )}
                    </div>
                    {!isDefault && (
                      <button
                        onClick={() => handleRemoveAccessEmail(em)}
                        className="p-1.5 sm:p-2 text-slate-400 hover:text-red-500 transition-all bg-slate-50 hover:bg-red-50 rounded-lg"
                      >
            ...
            <div className="pt-3 sm:pt-4 border-t border-slate-100">
              <button
                ...
                className="w-full px-4 sm:px-5 py-3 sm:py-4 rounded-xl sm:rounded-2xl font-black text-[10px] sm:text-xs uppercase text-slate-500 bg-slate-100 hover:bg-slate-200 active:scale-95 transition-all"
              >
                Tutup Jendela Ini
              </button>
            </div>
