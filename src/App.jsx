...
<head>
    ...
    <title>Aplikasi Dokumentasi Lapangan</title>
    
    <script src="https://cdn.tailwindcss.com"></script>
    
    <!-- Mencegah konflik AMD loader (RequireJS) pada lingkungan editor yang menyebabkan script UMD tidak ter-load ke window global -->
    <script>
        window.__tempDefine = window.define;
    ...
    <!-- Fix untuk lucide-react yang mencari variabel 'react' (huruf kecil) di lingkup global -->
    <script>window.react = window.React;</script>
    
    <script src="https://cdn.jsdelivr.net/npm/lucide-react@0.292.0/dist/umd/lucide-react.min.js"></script>
    
    <!-- Mengembalikan settingan AMD -->
    <script>
    ...
    <script type="module">
        ...
        import { getFirestore, doc, setDoc, getDoc, collection, deleteDoc, onSnapshot, writeBatch, updateDoc } from "https://www.gstatic.com/firebasejs/11.6.1/firebase-firestore.js";

        window._firebaseApp = { initializeApp, getApp, getApps };
        window._firebaseAuth = { getAuth, signInAnonymously, onAuthStateChanged };
        window._firebaseFirestore = { getFirestore, doc, setDoc, getDoc, collection, deleteDoc, onSnapshot, writeBatch, updateDoc };
...
<body>
    ...
    <script type="text/babel">
        // Fungsi pembungkus utama agar script tidak crash sebelum ikon diload
        window.renderApp = () => {
        
        const { useState, useEffect, useMemo, useRef } = React;
        const { 
          Camera, Trash2, Image: ImageIcon, Upload, FileDown, Presentation, 
        ...
        const { initializeApp, getApp, getApps } = window._firebaseApp;
        const { getAuth, signInAnonymously, onAuthStateChanged } = window._firebaseAuth;
        const { getFirestore, doc, setDoc, getDoc, collection, deleteDoc, onSnapshot, writeBatch, updateDoc } = window._firebaseFirestore;

        // Pastikan AppID selalu menggunakan milik Anda
        ...
        const AppWrapper = () => {
            ...
            const [lockedProjectWarning, setLockedProjectWarning] = useState(null);
            ...
            const saveToCloudNow = async (id, info, pagesObj, type, emailToSave, timeToSave) => {
                ...
                const finalEmail = emailToSave || activeEmail;
            ...
            // EFFECT UNTUK HEARTBEAT KUNCI DOKUMEN (SEDANG DIEDIT)
            useEffect(() => {
                if (!activeProjectId || !activeEmail || !db || isOfflineMode) return;
                ...
                const updateLockStatus = async (isLocked) => {
                    try {
                        ...
                        await updateDoc(projRef, {
                            lockedBy: isLocked ? activeEmail : null,
                            lockedAt: isLocked ? Date.now() : null
                        });
                    } catch (e) {
                        // Abaikan error jika dokumen baru dibuat dan belum ada di cloud
                    }
                };
            ...
            }, [view, activeProjectId, activeEmail, isOfflineMode]);
            ...
            const handleMegaUpload = async (e) => {
                ...
                const currentPageArray = safeArray(typePages[curIdx] || createNewPage()[0]);
            ...
            const copyRulesToClipboard = () => {
                ...
                setStatusMsg({ text: 'Kode Rules disalin!', type: 'success' });
                setTimeout(() => setStatusMsg({ text: '', type: '' }), 2000);
                } catch (err) {
                setStatusMsg({ text: 'Gagal menyalin', type: 'error' });
                }
                document.body.removeChild(textArea);
            };
            ...
            return (
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        ...
                        return (
                            <div key={p.id} className="bg-white rounded-[24px] p-5 shadow-xl border border-slate-200 hover:border-blue-400 transition-all flex flex-col hover:-translate-y-1 relative">
                                ...
                                {(() => {
                                    const isLocked = p.lockedBy && p.lockedBy !== activeEmail && (Date.now() - (p.lockedAt || 0)) < 45000;
                                    
                                    if (isLocked) {
                                        return (
                                            <button onClick={() => setLockedProjectWarning(p)} className="w-full bg-red-50 hover:bg-red-100 text-red-600 border border-red-200 py-2.5 rounded-xl font-black text-[10px] uppercase transition-all flex justify-center items-center gap-2 animate-pulse shadow-sm">
                                                <Activity size={14}/> SEDANG DIEDIT
                                            </button>
                                        );
                                    }
                ...
                {view === 'edit' && (
                    ...
                    <section className="bg-white rounded-[32px] p-5 sm:p-8 shadow-2xl mb-8 border border-slate-200/60">
                        ...
                        <div className="grid grid-cols-1 gap-4 sm:gap-6 uppercase">
                        ...
                        {(reportInfo.customMeta || []).map((meta, idx) => (
                            <div key={meta.id} className="w-full group relative">
                ...
                {lockedProjectWarning && (
                    <div className="fixed inset-0 z-[999] flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4">
                    <div className="bg-white rounded-[24px] p-6 sm:p-8 max-w-md w-full shadow-2xl animate-in zoom-in-95 border-2 border-red-100">
                        <div className="flex items-center gap-3 text-red-600 mb-4">
                            <div className="bg-red-100 p-2 rounded-full"><Activity size={24} className="animate-pulse"/></div>
                            <h3 className="text-lg font-black uppercase tracking-widest">Sedang Diedit</h3>
                        </div>
                        <p className="text-slate-600 font-medium mb-2 text-xs sm:text-sm leading-relaxed">
                            Laporan ini sedang dibuka dan diedit oleh: <br/><strong className="text-slate-800 bg-slate-100 px-2 py-1 rounded inline-block mt-1">{lockedProjectWarning.lockedBy}</strong>
                        </p>
                        <p className="text-red-500 text-[10px] font-bold mb-6">Membuka paksa laporan ini dapat menyebabkan data saling tertimpa (konflik).</p>
                        <div className="flex gap-3">
                            <button onClick={() => setLockedProjectWarning(null)} className="flex-1 py-3 font-black bg-slate-100 hover:bg-slate-200 text-slate-600 rounded-xl uppercase text-[10px] transition-all">Tutup</button>
                            <button onClick={() => { openProject(lockedProjectWarning); setLockedProjectWarning(null); }} className="flex-1 py-3 font-black bg-red-50 hover:bg-red-100 border border-red-200 text-red-600 rounded-xl uppercase text-[10px] shadow-sm transition-all flex items-center justify-center gap-2">Buka Paksa <ChevronRight size={14}/></button>
                        </div>
                    </div>
                    </div>
                )}
        ...
        // Render aplikasi di dalam pembungkus window.renderApp
        const root = ReactDOM.createRoot(document.getElementById('root'));
        root.render(<AppWrapper />);
        
        }; // Penutup window.renderApp

        // Menunggu semua resource siap secara berkala (Safe Boot)
        const bootApp = setInterval(() => {
            if (window._firebaseLoaded && window.lucideReact && window.lucideReact.Camera) {
                clearInterval(bootApp);
                window.renderApp();
            } else if (window._firebaseLoaded && window.LucideReact && window.LucideReact.Camera) {
                // Fallback jika UMD mengekspor dengan nama kapital LucideReact
                window.lucideReact = window.LucideReact;
                clearInterval(bootApp);
                window.renderApp();
            }
